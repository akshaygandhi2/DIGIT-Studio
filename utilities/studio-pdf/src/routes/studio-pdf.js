...
router.post(
  ...
  asyncMiddleware(async (req, res) => {
    ...
    if (!applicationNumber)
      return renderError(
        res,
        "applicationNumber is mandatory to generate the receipt",
        400
      );
    if (!pdfKey)
      return renderError(
        res,
        "pdfKey is mandatory to generate the receipt",
        400
      );
    ...
    try {
      ...
      try {
        applicationDetails = await getPublicServiceApplicationDetails(
          tenantId,
          serviceCode,
          applicationNumber,
          requestInfo
        ...
        logger.error(
          `Error fetching details for application ${applicationNumber} with service ${serviceCode}`
        );
        logger.error(err);
        return renderError(
          res,
          "Failed to query details of the application",
          500
        );
      }
      ...
      if (application?.length > 0) {
        try {
          logger.info(
            `Generating PDF with key ${pdfKey} for application ${applicationNumber}`
          );
          ...
          logger.info(
            `PDFGenerateObject data: ${JSON.stringify(PDFGenerateObject)}`
          );
          ...
          return renderError(
            res,
            "Failed to generate PDF for application",
            500
          );
        }
      } else {
        ...
        return renderError(
          res,
          "No application found for the given applicationNumber",
          404
        );
      }
    } catch (err) {
